...
% = Framework (~8 pages) = 
% -- Part 1 
%    [part 1, what's done so far (larger)] 
%    [part 1, proposed work (a few paragraphs?)} 
% -- Part 2 
%    [part 2, what's done so far (larger)] 
%    [part 2, proposed work (a few paragraphs?)} 
% -- Part 3 
%    [part 3, what's done so far (larger)] 
%    [part 3, proposed work (a few paragraphs?)} 
% 
% = Research Questions (~4 pages) = 
% A list of research questions that you propose to answer in your thesis.  These can be theoretical questions, or experiments that you want to run, or demonstrations you want to do on the robot. 
% 
% = Timeline (~1 page) = 
% A table.  During these months, I will address research questions Q1 and Q3, etc. 
% 
% The astute reader will notice that this doesn't sum to 25 pages.  Presumably a title page, abstract, references, etc will fill it out.  Sidd, what do you think, did I get the allocations about right? 
...
estimate the joint angles of the robot instead; and hopefully arrive at a better
pose estimate this way. If 3D sensors (such as RGB-D cameras, or lasers) are
available, the solution is related to dense 3D odometry. We can use
kinematically-constrained gradient descent to produce a pose estimate
consistent with both the robot kinematics and the map. If only 2D sensors
(cameras) are available, we can track features between each image frame, and
estimate the joint angles of the robot which would produce the observed image motion.
